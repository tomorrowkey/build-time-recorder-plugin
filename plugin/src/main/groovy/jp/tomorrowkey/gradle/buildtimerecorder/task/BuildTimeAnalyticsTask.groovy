package jp.tomorrowkey.gradle.buildtimerecorder.task

import jp.tomorrowkey.gradle.buildtimerecorder.BuildTimeRecorderExtension
...
class BuildTimeAnalyticsTask extends DefaultTask {
    @TaskAction
    void run() {
        def content = getResourceContent("resource.txt")
        println(content)

        def extension = project.extensions.getByType(BuildTimeRecorderExtension)
